...
describe('getTransactionCount', () => {
  ...
  it('returns a transaction count', async () => {
    ...
    const transactionCount = await getTransactionCount(provider, sponsorWalletAddress, blockNumber, goOptions);
    expect(transactionCount).toEqual(10);
    expect(provider.rpcProvider.getTransactionCount).toHaveBeenCalledWith(sponsorWalletAddress, blockNumber);
  });
